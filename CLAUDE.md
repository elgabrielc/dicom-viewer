...
- **Track bugs in the private `myradone-internal` repo.** Active bug tracking lives in [`elgabrielc/myradone-internal`](https://github.com/elgabrielc/myradone-internal) -- both `BUGS.md` and GitHub Issues. The public `docs/BUGS.md` here is a pointer file only. Document each new bug with full context:
  ...
  External contributor bug reports still go to public Issues on `dicom-viewer`. Move them into the private tracker if they need durable internal context. PRs in this (public) repo cannot auto-close issues in the private repo; close those manually after the fix merges.
- **Track code review findings in CODE_REVIEWS.md.** Document findings from PR reviews in `docs/CODE_REVIEWS.md`, organized by PR with severity grouping (Critical/Important/Suggestions). Update finding status as issues are resolved. Promote unresolved findings to the private bug tracker if they survive past merge.
